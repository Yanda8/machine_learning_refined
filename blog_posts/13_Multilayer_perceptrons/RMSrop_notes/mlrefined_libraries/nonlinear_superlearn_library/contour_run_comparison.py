...
class Visualizer:
    ...
    def show_paths(self,g,w_histories,labels,**kwargs):
        self.g = g
        self.w_hist = w_histories
        ...
        num_histories = len(w_histories)
        fig, axs = plt.subplots(num_histories, 1, figsize=(7,7))
        gs = gridspec.GridSpec(num_histories, 1) 
        
        # loop over histories
        for j in range(num_histories):
            w_hist = w_histories[j]
            ax = plt.subplot(gs[j]) #,aspect = 'equal'); 

            # draw contours
            self.draw_contour_plot(ax,fig,w_hist)
    ...
    def draw_contour_plot(self,ax,fig,w_hist):
        ...
        ymax = []
        ymin = []
        for h in self.w_hist:
            x1 = max([v[0] for v in h])[0]
            xmax.append(x1)
            x2 = min([v[0] for v in h])[0]
            xmin.append(x2)
            
            x1 = max([v[1] for v in h])[0]
            ymax.append(x1)
            x2 = min([v[1] for v in h])[0]
            ymin.append(x2)
