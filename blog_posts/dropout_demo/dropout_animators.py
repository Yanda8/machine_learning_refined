...
class Visualizer:
    ...
    def animate_1d_regression(self,run,frames,**kwargs):
        ...
        train_cost_history = run.train_cost_histories[0]
        test_cost_history = run.test_cost_histories[0]
        train_ind_history = run.train_ind_histories[0]
        test_ind_history = run.test_ind_histories[0]
        ...
        def animate(k):
            ...
            train_inds = train_ind_history[current_ind]
            test_inds = test_ind_history[current_ind]
            
            # produce static img
            show_fit = False
            if k > 0:
                show_fit = True
            self.show_1d_regression(ax,w_best,train_inds,test_inds,run,scatter,show_fit)
    ...
    # 1d regression demo
    def show_1d_regression(self,ax,w_best,train_inds,test_inds,runner,scatter,show_fit):
        cost = runner.cost
        ...
        ax.scatter(self.x[train_inds].flatten(),self.y[train_inds].flatten(),color = self.colors[0],s = 70,edgecolor = 'k',linewidth = 1)
        ax.scatter(self.x[test_inds].flatten(),self.y[test_inds].flatten(),color = self.colors[1],s = 70,edgecolor = 'k',linewidth = 1)
