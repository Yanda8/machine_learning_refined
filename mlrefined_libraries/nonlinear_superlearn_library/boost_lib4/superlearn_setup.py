...
class Setup:
    ...
    def boost(self,num_rounds,**kwargs):                        
        ...
        self.best_trons.append(copy.deepcopy(model))
        self.models.append(copy.deepcopy(model))
        model = lambda x,trons=self.best_trons: np.sum([v(x) for v in trons])
        ...
        self.best_weights = []
        self.best_weights.append(w_best)
        
        # define output
        C = 1
        if self.cost_name == 'multiclass_softmax' or self.cost_name  == 'multiclass_perceptron':
            C = np.unique(self.y)
        ...
        for i in range(num_rounds):          
            ...
            scale = 0.1
            U = 10
            w = [scale*np.random.randn(self.x.shape[0] + 1,U), scale*np.random.randn(U,C)]
    
            # construct model to test
            next_unit = lambda x,w: np.dot(self.perceptron(x,w[0]).T,w[1])
            ...
            self.best_trons.append(copy.deepcopy(best_perceptron))
