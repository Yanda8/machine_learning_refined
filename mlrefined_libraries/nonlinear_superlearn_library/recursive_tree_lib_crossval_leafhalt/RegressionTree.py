...
class RTree:
    def __init__(self,csvname,depth,**kwargs):
        ...
        stump = RegressionStump.Stump(self.x_train,self.y_train,self.x_valid,self.y_valid)
    ...
    def build_subtree(self,stump):    
        ...
        left_x_val = stump.left_x_val
        right_x_val = stump.right_x_val
        left_y_val = stump.left_y_val
        right_y_val = stump.right_y_val

        # make left and right stump
        left_stump_temp = stump
        right_stump_temp = stump
        if np.size(left_y) > 1:
            left_stump_temp = RegressionStump.Stump(left_x,left_y,left_x_val,left_y_val)
        if np.size(right_y) > 1:
            right_stump_temp = RegressionStump.Stump(right_x,right_y,right_x_val,right_y_val)
            
        # do we keep this split? - based on decreasing validation error
        if (left_stump_temp.val_error + right_stump_temp.val_error) < stump.val_error:
            left_stump = left_stump_temp
            right_stump = right_stump_temp
        else:
            left_stump = stump
            right_stump = stump
