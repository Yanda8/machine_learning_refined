...
class Setup:
    ...
    def fit(self,**kwargs):
        ...
        # use initializer for the first instance 
        if np.shape(self.weight_histories)[0] == 0:
            self.w_init = self.initializer()
        # use the same initializations for the rest
        else:
            self.w_init = self.weight_histories[0][0]
